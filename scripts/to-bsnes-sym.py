...
state = State.WITHIN_UNKNOWN
for line in inData:
    line = line.strip()
    if line == '[labels]':
        state = State.WITHIN_LABELS
        continue
    elif line == '[definitions]':
        state = State.WITHIN_DEFS
        continue
    fields = line.split()
    if len(fields) != 2:
        continue
    if state == State.WITHIN_LABELS:
        # get address of each symbol
        symMap[fields[1]] = {'addr': int(fields[0], 16), 'size': 0}
    elif state == State.WITHIN_DEFS and fields[1].startswith('_sizeof_'):
        # get size of each symbol
        symMap[fields[1][8:]]['size'] = int(fields[0], 16)

# output bsnes compatible symbol file (see https://github.com/BenjaminSchulte/fma-snes65816/blob/master/docs/symbols.adoc)
...
''')
    for key in symMap:
        if key.startswith('SECTIONSTART_') or key.startswith('SECTIONEND_') or key.startswith('RAM_USAGE_SLOT_'):
            continue
        sym = symMap[key]
        if sym['size'] == 0:
            continue
        name = key if key != 'tcc__registers' else 'tcc_r0'
        f.write('{:02x}:{:04x} {:s} ANY {:d}\n'.format(sym['addr'] >> 16, sym['addr'] & 0xFFFF, name, sym['size']))
